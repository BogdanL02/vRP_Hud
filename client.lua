...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		TriggerEvent('vRP:setMoneyDisplay', 0.0)
		...
		if inVehicle == false then
			...
			if IsPedInAnyVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
			--	local essence = exports.vRP_AdvancedFuel:getFuel()
...
RegisterNetEvent('vRP:setAccountMoney')
AddEventHandler('vRP:setAccountMoney', function(account)
	if account.name == "bank" then
		SendNUIMessage({action = "setValue", key = "bankmoney", value = "$"..account.money})
	elseif account.name == "black_money" then
		SendNUIMessage({action = "setValue", key = "dirtymoney", value = "$"..account.money})
...
RegisterNetEvent('vRP_customui:updateStatus')
AddEventHandler('vRP_customui:updateStatus', function(status)
...
TriggerEvent('vRP_customui:updateWeight', hp - 100)
end
end)

RegisterNetEvent('vRP_customui:updateWeight')
AddEventHandler('vRP_customui:updateWeight', function(weightt)
